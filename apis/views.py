...
from .models import (Category, NewCollection, Options, Products, SubCategory,
                     SubSubCategory, cart)
...
class SubSubcategoryAPIView(APIView):
    def get(self, request, format=None):
        if self.request.query_params.get("filter"):
            ...
            filterProductsSerilizer = productSerializer(
                filterProducts, many=True, context={"request": request}
            )
            brandnames = []
            colors = []
            sizes = []
        ...
        serializer = SubSubcategorySerializer(
            subsubcategories, many=True, context={"request": request}
        )
...
class SubSubcategoryDetailAPIView(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        serializer = SubSubcategorySerializer(
            subsubcategory, context={"request": request}
        )
        return Response(serializer.data)
...
class NewArrivalViewset(APIView):
    def get(self, request, format=None):
        ...
        offerProductsSerilizer = productSerializer(
            offerProducts, many=True, context={"request": request}
        )
        brandnames = []
        colors = []
        sizes = []
    ...
    def get(self, request, format=None):
        ...
        if self.request.query_params.get("key"):
            ...
            products = Products.objects.filter(
                Q(name__startswith=key.upper()) | Q(name__startswith=key.lower)
            )
            if not products.exists():
                products = Products.objects.filter(
                    Q(name__icontains=secondKey.upper())
                    | Q(name__icontains=secondKey.lower())
                )
            if not products.exists():
                if key != "MEN" or "men":
                    products = Products.objects.filter(
                        Q(name__icontains=key.upper()) | Q(name__icontains=key.lower())
                    )
            if not products.exists():
                subsubcategoryIDs = []
                ...
                products = Products.objects.filter(
                    subsubcategory__id__in=subsubcategoryIDs
                )
            ...
            productserializer = productLessSerializer(
        ...
        for item in cartitems:
            ...
            if Offer.objects.filter(product=item.product).exists():
                ...
                if currency == "USD":
                    ...
                    unitprice = (
                        Offer.objects.get(product=item.product).OfferSterling * 100
                    )
                elif currency == "EUR":
                    unitprice = Offer.objects.get(product=item.product).OfferEuro * 100
                elif currency == "AED":
                    unitprice = (
                        Offer.objects.get(product=item.product).OfferDirham * 100
                    )
                else:
                    pass
            else:
    ...
    def post(self, request, format=None):
        ...
        if Offer.objects.filter(product_id=product).exists():
            ...
            if currency == "USD":
                ...
                unitprice = (
                    Products.objects.get(id=product).productpriceSterling
                ) * 100
            elif currency == "EUR":
...
class BottomProductAPIView(APIView):
    def get(self, request, format=None):
        ...
        if bproducts.exists():
            ...
            bottomProductsSerilizer = productSerializer(
                bottomProducts, many=True, context={"request": request}
            )
            brandnames = []
            colors = []
            sizes = []
